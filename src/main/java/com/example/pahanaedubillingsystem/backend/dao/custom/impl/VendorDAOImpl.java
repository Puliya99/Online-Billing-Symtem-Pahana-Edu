...
public class VendorDAOImpl implements CrudDAO {
    ...
    private static final String UPDATE_VENDORS = "UPDATE vendors SET name = ?, item_id = ?, description = ?, qty = ?, buying_price = ? WHERE grn_id = ?";
    ...
    public boolean save(Vendor entity) throws SQLException {
        return SQLUtil.execute(SAVE_VENDORS, entity.getGrnId(), entity.getName(), entity.getItemId(), entity.getDescription(), entity.getQty(), entity.getBuyingPrice());
    }
    ...
    public List<Vendor> getAll() throws SQLException {
        ...
        while (rst.next()) {
            vendors.add(new Vendor(rst.getString("grn_id"), rst.getString("name"), rst.getString("item_id"), rst.getString("description"), rst.getInt("qty"), rst.getDouble("buying_price")));
        }
        return vendors;
    }
